...
<body style="overflow: hidden;" scroll="no">
	<t:formvalid formid="formobj" layout="table" dialog="true">
		...
		<div class="datagrid-toolbar"
			<c:if test="${parentId eq '0'}">style="display:none"</c:if>>
			...
			<tbody>
				...
				<tr <c:if test="${parentId eq '0'}"></c:if>>
					...
					<td class="ng-binding"><input type="text" name="groupName"
						id="groupName" value="${functionGroup.groupName }"
						<c:if test="${parentId eq '0'}"> readonly="readonly"</c:if> /></td>
				</tr>
				...
				<tr <c:if test="${parentId ne '0'}"> style="display:none"</c:if>>
					...
					<td class="ng-binding"><input id="deptId" name="deptId"
						...
						<c:if test="${parentId eq '0'}">onclick="javascript:void(0)" readonly="readonly"</c:if>

...
<body style="overflow: hidden;" scroll="no">
	<t:formvalid formid="formobj" layout="table" dialog="true">
		...
		<input type="hidden" id="deptName1" name="deptName1"
			value="${deptName}" />
		<input type="hidden" id="deptCode1" name="deptCode1"
			value="${deptCode}" />
		<input type="hidden" id="level" name="level"
			value="${functionGroup.level}" />
		<table class="table-form" cellspacing="0">
			<tbody>
				<c:set var="parentId" value="${functionGroup.pid}"></c:set>
				<c:set var="curId" value="${functionGroup.id}"></c:set>
				<tr
					<c:if test="${(empty curId and empty pid) or (not empty curId and parentId eq '0')}"> style="display:none"</c:if>>
					<th><span>上级权限组名称:</span></th>
					<td class="ng-binding"><input type="hidden" id="pid"
						name="pid" value="${functionGroup.pid}" /> <label id="pGroupName">${pGroupName}</label>
					</td>
				</tr>
				<tr>
					<th style="width: 350px;"><span>权限组名称:</span></th>
					<td class="ng-binding"><input type="text" name="groupName"
						id="groupName" value="${functionGroup.groupName }" /></td>
				</tr>

				<tr <c:if test="${not empty pid}"> style="display:none"</c:if>>
					<th><span>部门名称:</span></th>
					<td class="ng-binding"><input id="deptId" name="deptId"
						type="hidden" value="${functionGroup.deptId}"> <input
						id="deptName" name="deptName" type="text"
						<c:if test="${parentId ne '0' and not empty parentId}">onclick="javascript:void(0)" readonly="readonly"</c:if>
						value="${functionGroup.deptName}" onclick="openDepartmentSelect()">
					</td>
				</tr>
				<tr style="display: none">
					<th><span>部门编码:</span></th>
					<td class="ng-binding"><input type="text" name="deptCode"
						id="deptCode" value="${functionGroup.deptCode }"
